...
const adminSetResourceMapEth = async () => {
    try {
        ...
        const tx = await bridge.methods
            .adminSetResourceMap(
                ...
                tokens.map((tkn) => tkn.ResourceId), // Resource IDs, bytes32[] memory
                tokens.map((tkn) => tkn.EthereumTokenAddress) // Token addresses, address[] memory
            )
            .send({
                from: accounts[0]
            })
        ...
        console.error('An error occurred in adminSetResourceMapEth():\n', error)
    }
}

adminSetResourceMapEth()
